 ...
 ##       for rtt in 5 10 25 50 75 100 150 200 
        ...
        do
            ...
            do 
                ...
                sudo tc qdisc replace dev $(ip route get 10.10.1.1 | grep -oP "(?<=dev )[^ ]+") root netem delay "$delay"ms
                ## Create an htb qdisc
                ...
                sudo ssh -o StrictHostKeyChecking=no -T root@h1 "iperf3 -c h3 -C cubic -n 10mb -fk > ./fig6/"$bufcap"_"$bandwidth"_"$delay"_"$trial"_cubic_10.txt"
                sudo ssh -o StrictHostKeyChecking=no -T root@h3 "iperf3 -s -1 -D"
                sudo ssh -o StrictHostKeyChecking=no -T root@h1 "iperf3 -c h3 -C bbr -n 10mb -fk > ./fig6/"$bufcap"_"$bandwidth"_"$delay"_"$trial"_bbr_10.txt"
                sudo ssh -o StrictHostKeyChecking=no -T root@h3 "iperf3 -s -1 -D"
                sudo ssh -o StrictHostKeyChecking=no -T root@h1 "iperf3 -c h3 -C cubic -n 100mb -fk > ./fig6/"$bufcap"_"$bandwidth"_"$delay"_"$trial"_cubic_100.txt"
                sudo ssh -o StrictHostKeyChecking=no -T root@h3 "iperf3 -s -1 -D"
                sudo ssh -o StrictHostKeyChecking=no -T root@h1 "iperf3 -c h3 -C bbr -n 100mb -fk > ./fig6/"$bufcap"_"$bandwidth"_"$delay"_"$trial"_bbr_100.txt"
            done
        done
    done
done