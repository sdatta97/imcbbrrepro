...
cleanup ()
{
    ...
    # losses
	loss=$(cat sender-ss.txt |   sed -e ':a; /<->$/ { N; s/<->\n//; ba; }' | grep "ESTAB"  | grep -oP '\bunacked:.*\brcv_space'  | awk -F '[:/ ]' '{print $7}' | tr -d ' ')

	# get cwnd, ssthresh
	cwn=$(cat sender-ss.txt |   sed -e ':a; /<->$/ { N; s/<->\n//; ba; }' | grep "ESTAB"    |  grep "unacked" | grep -oP '\bcwnd:.*(\s|$)\bbytes_acked' | awk -F '[: ]' '{print $2","$4}')

    # get bytes acked, unacked
    ack=$(cat sender-ss.txt |   sed -e ':a; /<->$/ { N; s/<->\n//; ba; }' | grep "ESTAB"  |  grep "unacked" | grep -oP '\bbytes_acked:.*(\s|$)\bsegs_out' | awk -F '[: ]' '{print $2","$4}')


	# concatenate into one CSV
	paste -d ',' <(printf %s "$ts") <(printf %s "$send_q") <(printf %s "$sender") <(printf %s "$retr") <(printf %s "$loss") <(printf %s "$cwn") <(printf %s "$ack") > sender-ss.csv
