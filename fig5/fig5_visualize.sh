#!/bin/bash
rm -rf *.csv
echo "BBR_goodput,CUBIC_goodput,Buffer,Bandwidth,rtt" >> tput.csv
echo "BBR_retransmissions,CUBIC_retransmissions,Buffer,Bandwidth,rtt" >> retr.csv
for bufcap in 100 10000 
do
    for bandwidth in 10 20 50 100 250 500 750 1000 
    do
        for rtt in 5 10 25 50 75 100 150 200 
        do
            for trial in 1 2 3 4 5
            do
                expstr="$bufcap,$bandwidth,$rtt"
                bbr2_tput=$(rg sender "$bufcap"_"$bandwidth"_"$rtt"_"$trial"_bbr2.txt -A 0 | awk '{print $7}') 
                cubic_tput=$(rg sender "$bufcap"_"$bandwidth"_"$rtt"_"$trial"_cubic.txt -A 0 | awk '{print $7}') 
                echo "$bbr2_tput,$cubic_tput,$expstr" >> tput.csv  
                bbr2_retr=$(rg sender "$bufcap"_"$bandwidth"_"$rtt"_"$trial"_bbr2.txt -A 0 | awk '{print $9}') 
                cubic_retr=$(rg sender "$bufcap"_"$bandwidth"_"$rtt"_"$trial"_cubic.txt -A 0 | awk '{print $9}')   
                echo "$bbr2_retr,$cubic_retr,$expstr" >> retr.csv    
            done      
        done
    done   
done